...
def ngbr_max_capacity(example, hour, model):    # 80까지 늘림
    ...
    YYYY_a = pd.DataFrame(pred_a.mu, columns=['Predictions'])
    YYYY_a_sd = pd.DataFrame(pred_a.mu ** (1/2), columns=['Arrival Standard Deviation'])
    ...
    YYYY_d = pd.DataFrame(pred_d.mu, columns=['Predictions'])
    YYYY_d_sd = pd.DataFrame(pred_d.mu ** (1/2), columns=['Departure Standard Deviation'])
...
def plot_result(start, model, hour):   
    ...
    predictions = pd.DataFrame(y_a_pred.mu, columns=['Predictions'])
    predictions_sd = pd.DataFrame(y_a_pred.mu**(1/2), columns=['Standard Deviation'])
    ...
    predictions = pd.DataFrame(y_d_pred.mu, columns=['Predictions'])
    predictions_sd = pd.DataFrame(y_d_pred.mu ** (1/2), columns=['Standard Deviation'])
